...
public class BluetoothLeUart extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    public void send(String data) {
        if (data != null && !data.isEmpty()) {
            Log.d("tosee","this is the data:"+data);
...
//            return;
    ...
    public void onCharacteristicRead (BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            Log.d("disney", characteristic.getStringValue(0));
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
             Log.d("tryingtosuccess","Characteristic write successful");
        }
    ...
    public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if ( device.getAddress().equals("D8:C6:F2:1B:33:D9")){
            notifyOnParking();
        }
        // Connect to first found device if required.
        if (connectFirst && device.getAddress().equals("E5:C0:D1:73:20:EB")) {
